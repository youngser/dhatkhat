...
irm.multi = function(R, sweeps = 1000, prior.beta = 1, prior.gamma = 1, verbose = FALSE)
{
  ...
  # Sweeps.
  Z <- foreach (s = 1:sweeps, .combine='rbind', .export="z") %dopar% {
    # for (s in 1:sweeps) {
    for (i in 1:n) {
      if (verbose) cat("s = ", s, "/", sweeps, ", i = ", i, "/", n, "\n")
      # Propose changing this value by sampling from the existing CRP.
...
collectOut <- function()
{
  require(tidyverse)
  df <- NULL
  ofiles <- dir(".", pattern=glob2rx("*.RData"))
  nfiles <- length(ofiles)
  (nmc <- floor(nfiles/6)) # 100

  for (mc in 1:nmc) {
    for (i in 1:6) {
      load(paste0("Z-i",i,"-mc",mc,".RData"))
      df <- rbind(df, data.frame(i=i, mc=mc, ari= max(ari.mat)))
    }
  }
  df %>% group_by(i) %>% summarize(mean=mean(ari), sd=sd(ari))
  #     i  mean      sd
  # <dbl> <dbl>   <dbl>
  # 1     1 0.132 0.00534
  # 2     2 0.128 0.00585
